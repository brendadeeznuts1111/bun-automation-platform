...
describe("Bun.file properties", () => {
  test("size is 0 for non-existent file", () => {
    const f = Bun.file("/tmp/nonexistent-file-xyz123.txt");
    expect(f.size).toBe(0);
  });

  test("type defaults to text/plain;charset=utf-8", () => {
    const f = Bun.file("/tmp/test.txt");
    expect(f.type).toContain("text/plain");
  });

  test("name returns the file path", () => {
    const f = Bun.file("/tmp/test.txt");
    expect(f.name).toBe("/tmp/test.txt");
  });

  test("lastModified returns a number", () => {
    const f = Bun.file("/tmp/test.txt");
...
describe("Bun.write with various types", () => {
  ...
  test("writes number (converted to string)", async () => {
  ...
  test("writes boolean (converted to string)", async () => {
...
describe("Bun.write return value", () => {
  test("returns number (bytes written), not BunFile", async () => {
...
describe("Bug 35: Bun.write with object writes [object Object]", () => {
  test("object is NOT JSON stringified — writes [object Object]", async () => {
...
describe("Bug 36: Bun.write with ReadableStream writes [object ReadableStream]", () => {
  test("ReadableStream is NOT consumed — writes literal toString", async () => {
...
describe("file.writer() for streaming writes", () => {
  test("write/flush/end work correctly", async () => {
  ...
  test("flush() forces write to disk", async () => {
...
describe("file.slice()", () => {
  test("returns Blob with sliced content", async () => {
...
describe("createPath option", () => {
  test("auto-creates nested directories", async () => {
...
describe("large file handling", () => {
  test("1MB file writes and reads correctly", async () => {
...
describe("empty file", () => {
  test("size is 0 and text is empty string", async () => {
...
describe("directory handling", () => {
  test("Bun.file on directory has size > 0 but text() throws", async () => {

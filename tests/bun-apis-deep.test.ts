...
describe("Bun.password", () => {
  ...
  test.each(["argon2id", "argon2d", "argon2i", "bcrypt"] as const)(
  ...
  test("bcrypt cost affects hash", async () => {
...
describe("Bun.CSRF", () => {
  ...
  test.each(["sha256", "sha384", "sha512"] as const)("algorithm %s generates and verifies", (algorithm) => {
...
describe("Bun.color", () => {
  test("converts hex to hex", () => {
    expect(Bun.color("#ff5733", "hex")).toBe("#ff5733");
  });

  test("converts hex to uppercase HEX", () => {
    expect(Bun.color("#ff5733", "HEX")).toBe("#FF5733");
  });
...
describe("Bun.semver", () => {
  ...
  test("order handles prerelease", () => {
    expect(Bun.semver.order("1.0.0-beta", "1.0.0")).toBe(-1);
  });
});

describe("Bun.hash", () => {
  test("wyhash is deterministic", () => {
  ...
  test("wyhash accepts seed parameter", () => {
...
describe("Bun.Glob", () => {
  test("match returns true for matching pattern", () => {
    const glob = new Bun.Glob("*.ts");
    expect(glob.match("test.ts")).toBe(true);
  });

  test("match returns false for non-matching", () => {
    const glob = new Bun.Glob("*.ts");
  ...
  test("scan returns matching files", async () => {
    const glob = new Bun.Glob("*.ts");
...
describe("Bun.sleep", () => {
  ...
  test("sleep with Date resolves", async () => {
  ...
  test("sleep negative does not throw", async () => {
    await Bun.sleep(-1);
    expect(true).toBe(true);
  });
});
...
describe("Bun.sleepSync", () => {
  test("sleepSync(0) works", () => {
    Bun.sleepSync(0);
    expect(true).toBe(true);
  });

  test("Bug 34: sleepSync with Date throws (unlike Bun.sleep)", () => {
  ...
  test("sleepSync negative throws", () => {
    expect(() => Bun.sleepSync(-1)).toThrow();
  });
});

describe("Bun.peek", () => {
  test("returns value from resolved promise", () => {
    expect(Bun.peek(Promise.resolve("hi"))).toBe("hi");
  });

  test("returns same promise if pending", () => {
    const pending = new Promise(() => {});
    expect(Bun.peek(pending)).toBe(pending);
  });

  test("returns non-promise values as-is", () => {
  ...
  test("status returns fulfilled for resolved", () => {
    expect(Bun.peek.status(Promise.resolve(1))).toBe("fulfilled");
  });

  test("status returns pending for unresolved", () => {
    expect(Bun.peek.status(new Promise(() => {}))).toBe("pending");
  });

  test("status returns rejected for rejected", () => {
...
describe("Bun.randomUUIDv7", () => {
  ...
  test("buffer encoding returns 16-byte Buffer", () => {
    const buf = Bun.randomUUIDv7("buffer");
    expect(buf.length).toBe(16);
  });

  test("base64 encoding returns shorter string", () => {

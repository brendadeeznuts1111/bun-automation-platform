...
| **`Bun.spawnSync`** | Built-in | [Docs](https://bun.com/docs/runtime/spawn) | Stable | `const res = Bun.spawnSync(["echo", "hi"]);` | Synchronous process execution; captures stdout, stderr, and exitCode buffers. |
| **Bun Shell (`$`)** | Built-in | [Docs](https://bun.com/docs/runtime/shell) | Stable | `import { $ } from "bun"; const out = await $\`echo "hi"\`.text();` | Cross-platform shell with auto-escaping, pipes, `.json()`, `.lines()`, and `.nothrow()`. |
...
| **`Bun.CSRF`** | Built-in | [Docs](https://bun.com/docs/runtime/csrf) | Stable | `const token = Bun.CSRF.generate(secret, { expiresIn: 3600 }); if (Bun.CSRF.verify(token, { secret })) ...` | Generate/verify HMAC-signed CSRF tokens with expiration timestamps. |
...
| **`Bun.deflateSync` / `inflateSync`** | Built-in | [Guide](https://bun.com/docs/guides/util/deflate) | Stable | `const def = Bun.deflateSync(buf); const raw = Bun.inflateSync(def);` | Zero-dependency RFC 1951 DEFLATE compression. |
| **`Bun.gzipSync` / `gunzipSync`** | Built-in | [Guide](https://bun.com/docs/guides/util/gzip) | Stable | `const gz = Bun.gzipSync(buf); const raw = Bun.gunzipSync(gz);` | Zero-dependency Gzip compression with gzip headers. |
| **`Bun.hash` (CRC32, Adler32, Wyhash)** | Built-in | [Docs](https://bun.com/docs/runtime/bun-apis) | Stable | `const crc = Bun.hash.crc32(buf); const adler = Bun.hash.adler32(buf);` | Fast native checksums & hashing (`crc32`, `adler32`, `wyhash`, `cityHash64`). |
| **`Bun.escapeHTML`** | Built-in | [Guide](https://bun.com/docs/guides/util/escape-html) | Stable | `const safe = Bun.escapeHTML("<script>alert(1)</script>");` | Fast native HTML entity escaping for XSS prevention. |
| **`Bun.deepEquals`** | Built-in | [Guide](https://bun.com/docs/guides/util/deep-equals) | Stable | `if (Bun.deepEquals(objA, objB)) { ... }` | Fast recursive structural equality comparison. |
| **`Bun.sleep` / `Bun.sleepSync`** | Built-in | [Guide](https://bun.com/docs/guides/util/sleep) | Stable | `await Bun.sleep(1000);` | Promise-based sleep without `setTimeout` boilerplate; sync sleep available. |
| **`Bun.which`** | Built-in | [Guide](https://bun.com/docs/guides/util/find-executable-path) | Stable | `const bunPath = Bun.which("bun");` | Find executable path in `$PATH` across platforms. |
| **TypeScript `@types/bun` & Paths** | Built-in | [Guide](https://bun.com/docs/guides/runtime/typescript) · [Paths](https://bun.com/docs/guides/runtime/tsconfig-paths) | Stable | `"paths": { "@/*": ["./src/*"] }` | Built-in TypeScript compilation, bundler moduleResolution, and path aliases. |
| **Heap snapshots & stats (`bun:jsc`)** | Built-in | [Guide](https://bun.com/docs/guides/runtime/heap-snapshot) | Stable | `import { heapStats, generateHeapSnapshotForDebugging } from "bun:jsc";` | Heap profiling, GC statistics, and memory leak debugging via JavaScriptCore. |
| **`bun build --compile` + Codesign** | Built-in | [Guide](https://bun.com/docs/guides/runtime/codesign-macos-executable) | Stable | `codesign --entitlements entitlements.plist --sign "ID" ./server` | Compile single-file executable and sign with macOS Gatekeeper entitlements. |
